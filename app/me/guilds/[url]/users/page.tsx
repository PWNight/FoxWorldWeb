// app/me/guilds/[url]/users/page.tsx
"use client";
...
// Типизация данных пользователя гильдии
interface GuildUser {
    uid: number;
    nickname: string;
    permission: number;
...
export default function MyGuildMembers({ params }: PageProps) {
    ...
    const [currentUserPermission, setCurrentUserPermission] = useState<number>(0);
    ...
    const fetchGuildData = async (url: string, token: string): Promise<void> => {
    ...
    useEffect(() => {
        ...
        const initializeData = async () => {
            ...
            if (!guildUserResult.success || guildUserResult.data.permission < 1) {
                router.push("/me/guilds");
                return;
            }

            setCurrentUserPermission(guildUserResult.data.permission);
            await fetchGuildData(url, session.token);
    ...
    const handleUpdateUser = async (
        ...
        if (newPermission === 2) {
            const confirmTransfer = window.confirm(
                `Вы уверены, что хотите передать гильдию ${guildUrl} пользователю ${user.nickname}? Это сделает его новым главой, а вы потеряете права владельца.`
            );
            if (!confirmTransfer) {
                setIsLoading(false);
                return;
        ...
        if (!updateResult.ok) {
            ...
            setNotifyType("error");
            setNotifyMessage(
                `Произошла ошибка ${updateResult.status} при изменении уровня пользователя`
            );
            setIsLoading(false);
            return;
        }
        ...
        await fetchGuildData(guildUrl, session!.token);
    ...
    const handleDeleteUser = async (user: GuildUser): Promise<void> => {
        const confirmDelete = window.confirm(
            `Вы уверены, что хотите исключить ${user.nickname} из гильдии ${guildUrl}?`
        );
        if (!confirmDelete) {
            return;
        }
        ...
        if (!deleteResult.ok) {
            ...
            setNotifyType("error");
            setNotifyMessage(
                `Произошла ошибка ${deleteResult.status} при удалении пользователя`
            );
            setIsLoading(false);
            return;
        }
        ...
        const notifyResult = await sendNotification(
            user.uid,
            session!.token,
            `Вы были исключены из гильдии /${guildUrl}`
        );
        if (!notifyResult.success) {
            setNotifyMessage(
                `Произошла ошибка ${notifyResult.code || "неизвестная"} при отправке уведомления`
    ...
    const handleApplication = async (
        ...
        const status = isAccepted ? "Принята" : "Отклонена";
        ...
        if (!updateResult.ok) {
            ...
            setNotifyType("error");
            setNotifyMessage(
                `Произошла ошибка ${updateResult.status} при обработке заявки`
            );
            setIsLoading(false);
            return;
        }
        ...
        const notifyResult = await sendNotification(
            uId,
            session!.token,
            `Ваша заявка в гильдию /${guildUrl} была ${isAccepted ? "принята" : "отклонена"}.`
        );
        if (!notifyResult.success) {
            setNotifyMessage(
                `Произошла ошибка ${notifyResult.code || "неизвестная"} при отправке уведомления`
                            ...
                            <div className="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                                {Array(6)
                                    ...
                                    .map((_, key) => (
                                        <div
                                            key={key}
                                            className="bg-neutral-100 dark:bg-neutral-800 rounded-lg p-4 shadow animate-pulse"
                                        >
                    ...
                    {guildUsers.length > 0 ? (
                        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                            {guildUsers.map((user) => (
                                <div
                                    key={user.uid}
                                    className="bg-neutral-100 dark:bg-neutral-800 rounded-lg p-4 shadow hover:shadow-md transition-shadow"
                                >
                                    <div className="flex items-center gap-3 mb-4">
                                        <Image
                                    ...
                                    <div className="text-sm text-gray-600 dark:text-gray-400 mb-4">
                                        ...
                                        <p>
                                            Участник с: {new Date(user.member_since).toLocaleString("ru-RU")}
                                        </p>
                                    </div>
                                    <div className="flex gap-2 flex-wrap">
                                        ...
                                        {(currentUserPermission === 2 ||
                                            (currentUserPermission === 1 && user.permission === 0)) && (
                                            <>
                        ...
                        <div className="bg-neutral-100 dark:bg-neutral-800 rounded-lg p-6 shadow text-center">
                            ...
                            <p className="text-gray-600 dark:text-gray-400">
                                Активнее продвигайте свою гильдию!
                            </p>
                        </div>
                    )}
                </div>
                                    ...
                                    >
                                        ...
                                        <div className="text-sm text-gray-600 dark:text-gray-400 mb-4">
                                            <p>
                                                ...
                                                {application.about_user}
                                            </p>
                                            <p>
                                                <span className="font-medium">Почему выбрал:</span>{" "}
                                                {application.why_this_guild}
                                            </p>
                                            <p>
                                                <span className="font-medium">Дата заявки:</span>{" "}
                                                {new Date(application.create_data).toLocaleString("ru-RU")}
                                            </p>
                                        </div>
                                        <div className="flex gap-2">
                                            <Button
