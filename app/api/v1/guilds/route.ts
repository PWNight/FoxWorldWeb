    ...
    if (error){
        ...
        try{
            ...
            if (userGuilds.length > 0 && userGuilds[0].permission == 2){
                return NextResponse.json({status: false, message: 'Вы уже являетесь главой одной из гильдий'},{status:401})
            }

            const guildData : any = await query('SELECT * FROM guilds WHERE url = ?', [url])
            if(guildData.length != 0){
                return NextResponse.json({status: false, message: 'Гильдия с данной ссылкой уже существует'},{status:500})
            }
            ...
            return NextResponse.json({status: true, message: 'Гильдия успешно создана'},{status:200})
        }catch (error: any){
            return NextResponse.json({status: false, message: 'Internal Server Error', data: {errno: error.errno, sqlState: error.sqlState}}, {status:500})
        }
    }
